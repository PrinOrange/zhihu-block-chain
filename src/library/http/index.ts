...
import md5 from "md5";
import { RequestConfig } from "@root/src/config/request";
...
const lruCache = new LruCache({ max: 10000, ttl: 1000 * 3600 });
...
export async function asyncGenerateZhihuExtendsHeader({
  ...
  // 从cookie中提取d_c0
  const cookie_item_list = cookie
    .split(";")
    .map((item: string) => item.trim())
    .filter((item: string) => item.startsWith("d_c0"));
  const raw_d_c0 = cookie_item_list?.[0] || "";
  const cookie_d_c0 = raw_d_c0.split("d_c0=")?.[1] || "";

  // 解析出url中的pathname和query参数
  const rawUrlObj = new URL.URL(url);
  const rawUrlPathname = rawUrlObj.pathname;
  let rawUrlQuery: { [key: string]: any } = {};
  for (const key of rawUrlObj.searchParams.keys()) {
  ...
  if ([...Object.keys(rawUrlQuery)].length > 0) {
    ...
    encrypt_url = `${encrypt_url}?${querystring.stringify(
      rawUrlQuery,
      undefined,
      undefined,
      {
  ...
  // 执行加密
  const x_zst_96 = await asyncGetZhihuEncrypt({
...
export default class httpClient {
  ...
  static async get(url: string, config: AxiosRequestConfig = {}) {
    ...
    const extendHeader = await asyncGenerateZhihuExtendsHeader({
      url: url,
  ...
  static async downloadImg(url: string): Promise<Buffer> {
    const res = await httpInstance
      ...
      .catch((e) => {
        logger.log(
          `图片下载失败, url=>${url}, message:${e.message}, stack=>${e.stack}`
        );
        return { data: [] };
      });
    return res.data;
